...
- Pour toute modification TypeScript ou comportement applicatif : exécuter `npm run build`.
- Pour une modification documentaire seule : une relecture ciblée suffit, aucun build n'est requis.
- En cas de changement touchant le corpus ou `sessionStorage`, vérifier : import, rejet doublon/format/quota, restauration après rechargement, suppression unitaire et effacement complet de session.
- En cas de changement touchant la conversation, vérifier : bouton d'envoi désactivé/activé, affichage du message utilisateur, streaming du message assistant, restauration de l'état du compositeur après succès ou erreur.
...
- Pas de `any` évitable ni de contournement de TypeScript strict.
